...
def graph_success_rate():
    df_mbf, df_prophet, df_random, x_axis = get_data()
    ...
    plt.grid(True)
    plt.title('Delivery success rate MBF vs PROPHET vs Random strategies')
...
def graph_delay():
    ...
    delay_mbf = df_mbf['avg_delay'].tolist()
    delay_prophet = df_prophet['avg_delay'].tolist()
    delay_random = df_random['avg_delay'].tolist()
    ...
    # Plot ping_from values on the first y-axis
    ax1.plot(x_axis, delay_mbf, label='Delay MBF', marker='o', linestyle='-', color='blue')
    ax1.plot(x_axis, delay_prophet, label='Delay Prophet', marker='x', linestyle='-', color='red')
    ax1.plot(x_axis, delay_random, label='Delay Random', marker='+', linestyle='-', color='green')
    ax1.set_xlabel('h_factor')
    ax1.set_ylabel('Average packet delay')
    ...
    # ax2.plot(x_axis, hops_prophet, label='Average Hops Prophet', marker='x', linestyle='--', color='m')
    # ax2.plot(x_axis, hops_random, label='Average Hops Random', marker='^', linestyle='--', color='m')
    ...
    # plt.title(title)
    plt.grid(True)
    plt.title('Average number of packet delay (S)')
...
def graph_hops():
    ...
    # Create a second y-axis for number of hops
    ax1.plot(x_axis, hops_mbf, label='Average Hops MBF', marker='o', linestyle='--', color='blue')
    ax1.plot(x_axis, hops_prophet, label='Average Hops Prophet', marker='x', linestyle='--', color='red')
    ax1.plot(x_axis, hops_random, label='Average Hops Random', marker='^', linestyle='--', color='green')
    ax1.set_ylabel('Average number of hops', color='black')
    ax1.tick_params(axis='y', labelcolor='black')
    ax1.legend(loc='upper left') # , bbox_to_anchor=(1, 1)

    plt.grid(True)
    plt.title('Average number of hops')
...
def get_data():
    ...
    print(x_axis)
    return df_mbf, df_prophet, df_random, x_axis
